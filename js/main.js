...
function transtlit_url(){
  text = $('#title').val(); 
  $.get('scripts/text_translit.php', { text: text }, function(data){
    if ($('#url').val()==''){
      $('#url').val(data); 
    }
  });
...
function list_rows(page,level,sort,sc,fth,action,id){ 
  search = $('input#search').val();
  $.get('scripts/list_rows.php', { page: page, level: level, sort: sort, sc: sc, fth: fth, search: search, action: action, id: id }, function(data){
    $('#list_rows').html(data); 
  });
}
